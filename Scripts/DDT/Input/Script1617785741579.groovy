...
for (def loop = 1; loop <= findTestData('Input/Input_Reference').getRowNumbers(); loop++) //getRowNumbers permet de détecter automatiquement le nombres de lignes utiles dans le fichier Excel) 
	...
	if(count == 0) 
		...
		{
			WebUI.setText(findTestObject('Object Repository/Page_Google/input__q'), findTestData('Input/Input_Reference').getValue(3, loop)) //loop ici sert à définir sur quelle ligne du fichier Excel la donnée est récupérée
